...
def test_modelicapath_from_env(monkeypatch):
    """MODELICAPATH environment variable splits like any other specification"""
    monkeypatch.delenv("MODELICAPATH", raising=False)
    assert parser._modelicapath_from_env() == []
    monkeypatch.setenv("MODELICAPATH", os.pathsep.join(["/a", "/b"]))
    assert parser._modelicapath_from_env() == [Path("/a"), Path("/b")]
    # An explicit mapping overrides the process environment
    assert parser._modelicapath_from_env({"MODELICAPATH": "/c"}) == [Path("/c")]
...
def test_resolve_modelicapath_non_directory(monkeypatch, tmp_path):
    ...
    with pytest.raises(parser.ModelicaPathError, match="non-directory"):
        parser.resolve_modelicapath(tmp_path / "no_such_dir")
    with pytest.raises(parser.ModelicaPathError, match="non-directory"):
