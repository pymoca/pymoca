...
def assert_model_equivalent(A, B):
    def sstr(a):
    ...
    for vgroup in [
        ...
        assert sstr(getattr(A, vgroup)) == sstr(getattr(B, vgroup))


def assert_model_variables_equivalant(A, B):
    for vgroup in [
        "states",
        "der_states",
        "alg_states",
        ...
        for variable_this, variable_that in zip(variables_this, variables_that):
            assert variable_this.symbol.name() == variable_that.symbol.name()

            for attr in CASADI_ATTRIBUTES:
                val_this = getattr(variable_this, attr)
                val_that = getattr(variable_that, attr)

                assert isinstance(val_this, type(val_that)), "{}: '{}' ({})".format(
                    vgroup, variable_this.symbol.name(), attr
                )
                np.testing.assert_equal(val_this, val_that)

    assert A.delay_states == B.delay_states


def assert_model_string_parameters_constants_equivalent(A, B):
    assert len(A.string_constants) == len(B.string_constants)
    for variable_this, variable_that in zip(A.string_constants, B.string_constants):
        assert variable_this.to_dict() == variable_that.to_dict()

    assert len(A.string_parameters) == len(B.string_parameters)
    for variable_this, variable_that in zip(A.string_parameters, B.string_parameters):
        assert variable_this.to_dict() == variable_that.to_dict()


def assert_model_equivalent_numeric(A, B, tol=1e-9):
    assert len(A.states) == len(B.states)
    assert len(A.der_states) == len(B.der_states)
    assert len(A.alg_states) == len(B.alg_states)
    assert len(A.inputs) == len(B.inputs)
    assert len(A.outputs) == len(B.outputs)
    assert len(A.constants) == len(B.constants)
    assert len(A.parameters) == len(B.parameters)
    assert len(A.string_parameters) == len(B.string_parameters)
    assert len(A.delay_states) == len(B.delay_states)

    if not isinstance(A, CachedModel) and not isinstance(B, CachedModel):
        assert len(A.equations) == len(B.equations)
        assert len(A.initial_equations) == len(B.initial_equations)
        assert len(A.delay_arguments) == len(B.delay_arguments)
...
def test_if_else():
    ...
    ref_model.equations = [
        ...
        ca.if_else(
            ...
            ca.if_else(x > 2, ca.vertcat(y2 - y_max - 1, y3 - 1000), ca.vertcat(y2, y3 - 10000)),
        ),
    ]
...
def test_delay():
    ...
    assert_model_equivalent_numeric(ref_model, casadi_model)
    assert_model_equivalent_numeric(ref_model, casadi_model_expanded)


def test_delay_for_loop():
...
def test_delay_for_loop_with_expand_vectors():
...
def test_array_3d():
    ...
    for ind in np.ndindex(target_param_values.shape):
        ...
        assert param_symbols[flat_index].name() == "x[{}]".format(
            ",".join((str(x + 1) for x in ind))
        )
        assert casadi_model.parameters[flat_index].value == target_param_values[ind]
...
def test_array_expand():
    ...
    target_values = {}
    target_values["x"] = np.array([[-5.326999, 54.050758, 0.000000], [-1.0, 0.0, 0.0]], dtype=float)
    ...
    for v, target_param_values in target_values.items():
        for ind in np.ndindex(target_param_values.shape):
            ...
            val = variable.value
            assert val == target_param_values[ind], variable.symbol.name()
            assert isinstance(val, types[v]), variable.symbol.name()
...
def test_cache_metadata():
    ...
    assert_model_string_parameters_constants_equivalent(ref_model, cached_model)
    ...
    for a, b, c in zip(orig_metadata, function_metadata, cached_metadata):
        ...
        assert np.allclose(a_out, b_out, 0, 0, True)
        assert np.allclose(a_out, c_out, 0, 0, True)
...
def test_simplify_replace_constant_values():
    ...
    # Compare
    assert_model_string_parameters_constants_equivalent(casadi_model, ref_model)
    assert_model_equivalent_numeric(casadi_model, ref_model)


def test_simplify_replace_parameter_expressions():
    # Create model, cache it, and load the cache
...
def test_simplify_replace_parameter_values():
    # Create model, cache it, and load the cache
...
def test_simplify_eliminable_variable_expression():
    # Create model, cache it, and load the cache
...
def test_simplify_eliminable_variable_expression_if_else():
    # Create model, cache it, and load the cache
    compiler_options = {
...
def test_simplify_detect_aliases():
    # Create model, cache it, and load the cache
    ...
    assert_model_string_parameters_constants_equivalent(casadi_model, ref_model)
    assert_model_equivalent_numeric(casadi_model, ref_model)
    assert casadi_model.states[0].aliases == {"alias"}


def test_simplify_alias_small_nominal():
    # Create model, cache it, and load the cache
    compiler_options = {"detect_aliases": True}
    ...
    # Compare
    assert_model_equivalent_numeric(casadi_model, ref_model)
    assert casadi_model.alg_states[0].aliases == {"alias"}


def test_simplify_detect_negative_alias():
    # Create model, cache it, and load the cache
    compiler_options = {"detect_aliases": True}
...
def test_simplify_detect_alias_conflicting_start():
    # Create model, cache it, and load the cache
    compiler_options = {"detect_aliases": True}

    with __import__("unittest").TestCase().assertLogs("pymoca", level="WARNING") as cm:
        casadi_model = transfer_model(MODEL_DIR, "ConflictingAliasStart", compiler_options)

    assert "Current start attribute of canonical variable 'x' (p1) conflicts" in str(cm.output)
    assert (
    ...
    # Compare
    assert_model_equivalent_numeric(casadi_model, ref_model)
    assert casadi_model.states[0].aliases == {"-alias_neg", "alias_pos"}
...
def test_constant_aliases():
    ...
    assert c.aliases == {"z"}
    assert len(casadi_model.alg_states) == 0
...
def test_resolve_parameters():
    ...
    assert parameter_names == {"x", "z"}
    z = next(p for p in casadi_model.parameters if p.symbol.name() == "z")
    assert isinstance(z.value, ca.MX)
    assert not z.value.is_constant()
    assert z.value.name() == "x"
...
def test_resolve_parameters_expression_result_type():
    txt = """
    ...
    parameter_names = {p.symbol.name() for p in casadi_model.parameters}
    assert parameter_names == {"diameter", "area", "n", "n_squared"}

    area = next(p for p in casadi_model.parameters if p.symbol.name() == "area")
    assert isinstance(area.value, ca.MX)
    assert not area.value.is_constant()

    n_squared = next(p for p in casadi_model.parameters if p.symbol.name() == "n_squared")
    assert isinstance(n_squared.value, ca.MX)
    assert not n_squared.value.is_constant()
    ...
    area = next(p for p in casadi_model.parameters if p.symbol.name() == "area")
    assert not isinstance(area.value, ca.MX)
    assert isinstance(area.value, float)
    assert area.value == 0.25 * 3.14159265

    n_squared = next(p for p in casadi_model.parameters if p.symbol.name() == "n_squared")
    assert not isinstance(n_squared.value, ca.MX)
    assert isinstance(n_squared.value, int)
    assert n_squared.value == 4


def test_deleted_canonical_variable():
    txt = """
...
def test_simplify_expand_vectors():
    # Create model, cache it, and load the cache
...
def test_simplify_reduce_affine_expression_loop():
    ...
    assert "y[1]" in casadi_model.alg_states[0].aliases

    # Compare
    assert_model_equivalent_numeric(casadi_model, ref_model)


def test_derivative_aliases():
    txt = """
    ...
    assert casadi_model.der_states[0].aliases == {"z"}
    assert_model_equivalent_numeric(casadi_model, ref_model)


def test_no_derivative_aliases():
    txt = """
    ...
    assert casadi_model.der_states[0].aliases == set()
    assert_model_equivalent_numeric(casadi_model, ref_model)


def test_divide_constant_parameter_aliases():
    txt = """
    ...
    for expand_vectors in [False, True]:
        for expand_mx in [False, True]:
            ...
            assert casadi_model.alias_relation.aliases("x") == {"x", "y", "-z"}
            assert_model_equivalent_numeric(casadi_model, ref_model)


def test_divide_constant_parameter_vector_aliases():
    txt = """
...
def test_simplify_reduce_affine_expression():
    # Create model, cache it, and load the cache
...
def test_simplify_differentiated_state():
    # Create model, cache it, and load the cache
    compiler_options = {"eliminable_variable_expression": r"_\w+", "expand_mx": True}
...
def test_alias_relation():
    a = AliasRelation()
    assert a.canonical_signed("-a") == ("a", -1)
    a.add("a", "-b")
    a.add("b", "c")
    a.add("d", "-b")
    assert list(a) == [("d", {"a", "-b", "-c"})]
...
def test_cat_params():
    ...
    for i, e in enumerate(c):
        assert casadi_model.parameters[1].value[i] == e


def test_inline_input_assignment():
    casadi_model = transfer_model(MODEL_DIR, "InlineAssignment")
    assert casadi_model.inputs[0].fixed
    assert not casadi_model.alg_states[0].fixed
...
def test_unspecified_dimensions():
    ...
    assert a_x.symbol.name() == "a.x"
    assert a_x.value == [
        [88.3224, 281.642, 143.011],
        [58.8183, -24.9845, 0.0],
        [-1.45483, 0.0, 0.0],
    ]

    assert a_y.symbol.name() == "a.y"
    assert a_y.value == [[1, 2], [3, 4], [5, 6]]


def test_wrong_unspecified_dimensions():
    txt = """
        model A
          parameter Real x[:, :];
          parameter Real y[:, 4];
    ...
    assert not True, "No exception raised on wrong dimensionality."
...
def test_nested_indices_simple():
    txt = """
        model A
            Real x[3];
        end A;
...
def test_assigning_without_index():
    txt = """
        model Test
            Real x[3];
            Real y[3];
...
def test_assigning_without_index_nested():
    txt = """
        model A
            Real x[3];
        end A;
...
def test_nested_equations_indexing():
    txt = """
        model A
            Real x[3];
            Real y[3];
...
def test_nested_equations_indexing_hard_2d():
    txt = """
        model A
            Real x[3];
        end A;
...
def test_nested_equations_indexing_hard_3d():
    txt = """
        model A
            Real x[3];
        end A;
    ...
    ast_tree = parser.parse(txt)
    with pytest.raises(AssertionError, match="Dimensions higher than two are not yet supported"):
        casadi_model = gen_casadi.generate(ast_tree, "Test", {"expand_vectors": True})
        casadi_model.simplify({"expand_vectors": True})
...
def test_1based_indexing_error():
    txt = """
                    model Test
                        Real x[3];
                    equation
...
def test_nested_indexing_error():
    txt = """
        model A
            Real x[3];
        end A;
...
def test_index_at_wrong_symbol_error():
    txt = """
        model A
            Real x[3];
        end A;
...
def test_2d_index_at_wrong_symbol_error():
    txt = """
        model A
            Real x[3];
        end A;
...
def test_expand_vectors_derivative_naming():
    txt = """
        model A
          Real x[2];
        end A;
    ...
    assert der_names == ["der(a[1].x[1])", "der(a[1].x[2])"]
...
def test_unit_type():
    ...
    assert isinstance(casadi_model.parameters[0].value, int)
    assert isinstance(casadi_model.parameters[1].value, float)
    assert isinstance(casadi_model.parameters[2].value, float)
    assert isinstance(casadi_model.parameters[3].value, int)


def test_unit_type_vector():
    txt = """
        model A
          parameter Integer x = 1;
          parameter Real y = 1.0;
...
def test_unit_type_array():
    ...
    for i in range(0, 4):
        assert isinstance(casadi_model.parameters[i].value, int)
    for i in range(4, 8):
        assert isinstance(casadi_model.parameters[i].value, float)
...
def test_iterative_simplification():
    ...
    assert 1 == len(casadi_model.equations)

    casadi_model.simplify(compiler_options)


def test_iterative_simplification_large():
    compiler_options = {
        "eliminate_constant_assignments": True,
        "factor_and_simplify_equations": True,
        "replace_constant_expressions": True,
    ...
    assert 12 == len(casadi_model.alg_states)
...
def test_signed_expression():
    ...
    assert iplus.value == 1.0
    assert rplus.value == 1.0
    assert rneg.value == -1.0
    assert rboth.value == -2.0

    compiler_options = {"resolve_parameter_values": True}

    casadi_model.simplify(compiler_options)
    assert ineg.value == -1.0


def test_resolve_parameter_values_lists():
...
def test_modelicapath_option_resolves_library(monkeypatch):
    """The modelicapath option puts roots on the search path without the environment.

    Embedders that do not want to mutate os.environ pass their library roots as a
    list of directories or as one os.pathsep-separated string.
    """
    with tempfile.TemporaryDirectory() as model_dir, tempfile.TemporaryDirectory() as lib_dir:
        ...
        monkeypatch.delenv("MODELICAPATH", raising=False)
        for modelicapath in (lib_dir, [lib_dir], os.pathsep.join([lib_dir, model_dir])):
            model = transfer_model(
                model_dir,
                "UsesLib",
                {"library_folders": [], "cache": False, "modelicapath": modelicapath},
            )
            assert [p.symbol.name() for p in model.parameters] == ["x"]
...
def test_modelicapath_option_precedes_env(monkeypatch):
    ...
    with (
        ...
        # Without the option, the environment's root is the one that resolves
        model = transfer_model(model_dir, "UsesLib", {"library_folders": [], "cache": False})
        assert [p.symbol.name() for p in model.parameters] == ["fromEnv"]


def test_model_folder_class_shadows_modelicapath(monkeypatch):
    """A model-folder class shadows a same-named MODELICAPATH library (MLS 3.5 13.3).
