...
ahead of the `MODELICAPATH` environment variable's, the `-p` arguments for the CLI. Since
the first root to offer a top-level name wins, caller-supplied roots shadow same-named
libraries found in the environment. It also validates, raising `ModelicaPathError` on the
first root that is not a directory, and `modelicapath_to_tree` resolves its `dirs` through
it - so every consumer, CLI included, reports a bad root the same way, at the cost of
