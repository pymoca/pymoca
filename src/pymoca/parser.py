...
def _split_modelicapath(
    ...
    if modelicapath is None:
        entries: list[str | os.PathLike | None] = []
...
def _modelicapath_from_env(environ: Mapping[str, str] | None = None) -> list[Path]:
    """Return the directories in the MODELICAPATH environment variable ([] if unset)"""
    if environ is None:
        environ = os.environ
    return _split_modelicapath(environ.get(MODELICAPATH_ENV_VAR, ""))


def resolve_modelicapath(
    modelicapath: str | os.PathLike | Iterable[str | os.PathLike | None] | None = None,
    *,
    use_env: bool = True,
) -> list[Path]:
...
def modelicapath_to_tree(
    dirs: str | os.PathLike | Iterable[str | os.PathLike | None] | None = None,
    *,
    use_env: bool = False,
) -> ast.Tree:
