...
def find_name(
    ...
    :return: Class or Symbol found, or None if not found
...
def find_name_path(
    ...
    :param name: name to look up (can be a Class or Symbol name)
    :param scope: scope in which to start name lookup
    :return: TreePath to the found name or an empty TreePath if not found
    """

    return _find_name_path(name, scope)
...
def _find_name(
    ...
    found_name_path = _find_name_path(
        name,
        scope,
    ...
    return found_name_path.leaf if found_name_path else None
...
def _find_simple_name(
    ...
    if name in InstanceTree.BUILTIN_TYPES:
        # FIXME: Fix this temporary hack for finding BUILTIN_TYPES when scope is not an instance
        ast_ref = scope.root
        return TreePath(InstanceTree(ast_ref).classes[name])

    # Step 0.2: The scope itself
    if search_parent and scope.name == name:
        return _build_path(scope)
...
def _find_rest_of_name(first: TreePath, rest_of_name: str) -> TreePath:
    ...
    if isinstance(element, ast.Symbol):
        ...
        if isinstance(element.type, ast.Class):
            type_class = element.type
        else:
            assert element.parent, f"Symbol {first} has no parent"
            type_class_path = _find_name_path(element.type, element.parent)
...
def _flatten_first_and_find_rest(first: TreePath, rest_of_name: str) -> TreePath:
    ...
    found = _find_name_path(rest_of_name, first.leaf, search_parent=False)
    ...
    return first.splice_path(found) if found else TreePath()
...
def _find_local(
    ...
    if found := _find_iteration_variable(name, scope):
        return _build_path(found)

    # 2. Classes
    if name in scope.classes:
        return _build_path(scope.classes[name])
...
def _find_inherited(
    ...
    for extends in scope.extends:
        ...
        if isinstance(extends, ast.InstanceClass):
            found = _find_name_path(name, extends, current_extends=current_extends)
            if found:
                return _build_path(scope).append_path(found)
            else:
                return TreePath()

        # ExtendsClause case
        extends_scope = _find_name_path(
            extends.component,
            scope,
            current_extends=current_extends,
        )
        if extends_scope:
            ...
            if found:
                tree_path = _build_path(extends.parent) + TreePath(extends)
                tree_path = _join_extends_paths(tree_path, found)
                return tree_path
        else:
            current_extends.remove(extends)
    return TreePath()
