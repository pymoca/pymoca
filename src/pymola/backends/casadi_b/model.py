...
class Dae:
    ...
    @property
    def equation(self):
        return ca.vertcat(self.dae_eq, self.alg_eq)
    ...
    def __str__(self):
        n_v = self.unknown.shape[0]
        n_eq = self.equation.shape[0]
        line_width = 78
        s = '='*line_width + '\n'
        for name, group in [
            ...
            header = '-'*30 + '{:9s} ({:4d})'.format(name, np.sum([getattr(self, f).shape[0] for f in group]))
            ...
            s += header
            for f in group:
                s += '{:10s} {:4d}: {:s}\n'.format(f, getattr(self, f).shape[0] , str(getattr(self, f)))
        s += '='*line_width + '\n'
        return s
    ...
    def make_explicit(self):
        ...
        0 = f(\dot{x}, x, z, t) = M \dot{x} + A x + B z + f'(t)
        0 = g(x, z, t) = M \dot{x} + A x + B z + f'(t)

        If M is invertible can be transformed to semi-explicit form via:

        \dot{x} = -M^{-1}(A x + B z + f't)
        0 = g(x, z, t)
        ...
        \dot{x} = -M^{-1}(A - BD^{-1}C)x - M^{-1}(f'(t) - BD^{-1}f''(t))$
        \dot{x} = A'x + f'''(t)
        ...
        if ca.sprank(M) < x.shape[0]:
            print('cannot make explicit, mass matrix rank {:d} less than n_states {:d}'.format(M_rank, x.shape[0]))
            return False
        A = ca.jacobian(f, x)
        B = ca.jacobian(f, z)
        C = ca.jacobian(g, x)
        ...
        if ca.sprank(D) < z.shape[0]:
            print('cannot make explicit, dg/dz rank {:d} less than nz {:d}'.format(M_rank, z.shape[0]))
            return False

        # assert everything is linearizable
        if self.sym_class == ca.SX:
            assert ca.norm_2(ca.simplify(ca.mtimes(M, dx) + ca.mtimes(A, x) + ca.mtimes(B, z) + f_c - f)) == 0
            assert ca.norm_2(ca.simplify(ca.mtimes(C, x) + ca.mtimes(D, z) + g_c - g)) == 0

        # find new model
        f_Ap = ca.Function("Ap", [p, c], [-ca.mtimes([ca.inv(M), A - ca.mtimes([B, ca.inv(D), C])])])
        f_t = ca.Function("f_t", [p, c, t], [-ca.mtimes([ca.inv(M), f_c - ca.mtimes([B, ca.inv(D), g_c])])])
    ...
    def sim_ode(self, options: Dict=None):
        ...
        output_func = ca.Function('y', [self.state, self.parameter, self.time], [self.output_rhs])
        ...
        if ca.Importer.has_plugin('clang'):
          ...
          with_jit = False
          compiler = ''
        func_opts = {'jit': with_jit, 'compiler': compiler}

        # create integrator
        if opt['use_scipy']:
            f_ode = ca.Function('f',
                [self.time, self.state, self.parameter],
                [self.ode_rhs], func_opts)
            f_J = ca.Function('J', [self.time, self.state, self.parameter],
                [ca.jacobian(self.ode_rhs, self.state)], func_opts)
        ...
        for i in range(1, n):
            ...
            if opt['use_scipy']:
                ...
                # need to add t0, tf to casadi interface, this is  slow, see https://github.com/casadi/casadi/issues/1592
